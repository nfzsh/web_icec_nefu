...
{
	id=rs.getInt(1);
	out.println("<tr><td><li class='list-group-item'>"+rs.getString(3)+"</li></td><td align='right' width=200px><a href='delete2.jsp?id="+rs.getString(1)+"'>删除</td></tr>");
	if(!rs.next())
		break;
}
out.println("</table>");
...
页<input type="submit" class="button" class="btn btn-secondary" value="跳转"></form>
<hr>
<%
if(p==1){
	%>
 <a href=delete.jsp?p=<%=p+1 %>>下一页</a>   <a href=delete.jsp?p=<%=totalpage%>>尾页</a> 
<%}
else if(p==totalpage){
	%>	
	<a href=delete.jsp>首页</a>   <a href=delete.jsp?p=<%=p-1 %>>上一页</a> 
	
	<%
} else{
	%>
	<a href=delete.jsp?p=>首页</a>   <a href=delete.jsp?p=<%=p-1 %>>上一页</a> 
	<a href=delete.jsp?p=<%=p+1 %>>下一页</a>   <a href=delete.jsp?p=<%=totalpage%>>尾页</a> 
	<%
}
data.close();
